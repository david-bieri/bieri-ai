# WebDev Skills Manifest — Prof. David Bieri

*Web development skills optimized for Manus and Perplexity Computer execution.*

---

## Installed Skills

| Skill | Version | Status | Updated |
|-------|---------|--------|---------|
| `web-release-workflow` | 1.0.0 | Active | 2026-06-10 |
| `webdev-static-site-i18n` | 1.0.0 | Active | 2026-06-10 |
| `webdev-d3-analytics-modules` | 1.0.0 | Active | 2026-06-10 |
| `webdev-json-data-enrichment` | 1.0.0 | Active | 2026-06-10 |
| `webdev-latex-pdf-guide` | 1.0.0 | Active | 2026-06-10 |
| `webdev-cross-browser-smoke-test` | 1.0.0 | Active | 2026-06-10 |
| `webdev-contact-protocol-links` | 1.0.0 | Active | 2026-06-10 |
| `webdev-supabase-app` | 1.0.0 | Active | 2026-06-10 |
| `webdev-vite-express` | 1.0.0 | Active | 2026-06-10 |
| `webdev-deploy-render` | 1.0.0 | Active | 2026-06-10 |
| `webdev-platform-migration` | 1.0.0 | Active | 2026-06-10 |

---

## Active Projects

| Project | Prefix | Session notes | Registry |
|---------|--------|--------------|---------|
| AGW Conference Website | AGW | `AGW_SESSION_NOTES.md` | `AGW_PROGRESS.md` |
| Bieri Family Hub | FAM | `HOME_SESSION_NOTES.md` (domains/home/) | [bieri-family-hub](https://github.com/david-bieri/bieri-family-hub) |

---

## Skill Groupings

### Core Stack
`webdev-vite-express` → `webdev-supabase-app` → `webdev-deploy-render`

### Migration Path
`webdev-platform-migration` (orchestrates the above for cloud handoff)

### Existing WebDev Skills
`webdev-static-site-i18n`, `webdev-d3-analytics-modules`, `webdev-json-data-enrichment`, `webdev-latex-pdf-guide`, `webdev-cross-browser-smoke-test`, `webdev-contact-protocol-links`

---

## Update Log

| Date | Action | Detail |
|------|--------|--------|
| 2026-06-10 | Added | `webdev-supabase-app` v1.0.0 — schema, RLS, migrations, real-time |
| 2026-06-10 | Added | `webdev-vite-express` v1.0.0 — Vite + React + Express monorepo |
| 2026-06-10 | Added | `webdev-deploy-render` v1.0.0 — Render Web Service + Cron |
| 2026-06-10 | Added | `webdev-platform-migration` v1.0.0 — portability audit + migration |
